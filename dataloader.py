...
class DataLoader:
    def __init__(self, config_file, submissions_file):
        ...
        self.optional_params = [
            param for param in self.system_info.keys() if param not in ["id", "name"]
        ]
    ...
    def change_col_names(self):
        ren_col = {
            i: f"{i} ({int(self.questions_data_df[i]['Weight'] * 100)})"
            for i in self.questions_data_df.columns
        }
        ren_col["total"] = "Total (%)"
    ...
    def merge_match_res(self, write_mode="outer"):
        if self.match_list is not None and self.results is not None:
            if isinstance(self.match_list.columns[0], str):
                new_col_match_list = pd.MultiIndex.from_product(
                    [["Info"], self.match_list.columns]
                )
                ...
                raise Exception(
                    f"Unknown format of columns {type(self.match_list.columns)}"
                )
    ...
    def load_submissions(self):
        ...
        if file_ext == ".xlsx":
            file = pd.read_excel(
                self.submissions_file, dtype=str, keep_default_na=False
            )
        return file
    ...
    def load_match_list(self):
        ...
        if file_ext == ".xlsx":
            ...
            if not self.user_inputs["id"] in file.columns:
                ...
                file[self.user_inputs["id"]] = (
                    file[self.user_inputs["id"]].str.lower().replace(" ", "")
                )
    ...
    def filter_submissions(self):
        ...
        if time_:
            self.submissions[time_] = pd.to_datetime(
                self.submissions[time_], format=date_format
            )
            self.submissions["time_column"] = self.submissions[time_].dt.time
            if self.user_inputs.get("take_first_submission", False):
                self.submissions = self.submissions.loc[
                    self.submissions.groupby(id_)["time_column"].idxmin()
                ]
            else:
                self.submissions = self.submissions.loc[
                    self.submissions.groupby(id_)["time_column"].idxmax()
                ]
    ...
    def process_questions(self):
        ...
        for i, question in enumerate(questions["Questions"].values, start=1):
            ...
            metadata = (
                q_info.get("metadata", [])
                if q_info.get("metadata", []) is not None
                else []
            )
            metadata = [str(i) for i in metadata]
            questions_data_df[f"q{i}"] = [
                question,
